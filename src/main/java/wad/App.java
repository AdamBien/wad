...
import static com.airhacks.wad.control.PreBuildChecks.getBuilder;
...
public class App {
	...
	try (InputStream resourceAsStream = App.class.getClassLoader()
    ...
    static List<Path> addWarName(Set<Path> deploymentDirectories, Path warName) {
	return deploymentDirectories.stream().map(path -> path.resolve(warName.getFileName()))
	...
	WADFlow wadFlow = new WADFlow(sourceCodeDir, thinWARPath, deploymentTargets, builder);
    }
